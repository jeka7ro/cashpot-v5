...
echo "🔍 Updating slot machine providers based on Google Sheet mapping..."
...
for serial in "${EGT_SERIALS[@]}"; do
  ...
  if [ ! -z "$SLOT_ID" ]; then
    ...
    if [[ $UPDATE_RESPONSE == *"success"* ]] || [[ $UPDATE_RESPONSE == *"updated"* ]] || [[ $UPDATE_RESPONSE == *"id"* ]]; then
...
echo "🎉 Provider mapping update completed!"